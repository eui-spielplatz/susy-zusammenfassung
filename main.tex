...
\begin{tabular}{r >{\centering\arraybackslash} p{6cm}p{4cm}}
...
\begin{tabular}{r>{\centering\arraybackslash}p{6cm}l}
	...
	1 & \text{für } n=0\\
	-1 & \text{für } n=\pm1,\pm2,...\\
	\end{cases}$ & \raisebox{-.5\height}{\includegraphics[width=3cm]{img/zeitdiskImpulsfunktion}}\\ 
	& $\delta(n) = u(n)-u(n-1)$&\\
	\textbf{Rechteckfunktion} & $rect(\frac{n}{N})=\begin{cases}
	1 & \text{für } \vert n\vert \leq N\\
	0 & \text{für } \vert n\vert > N\\
	\end{cases}$ & \raisebox{-.5\height}{\includegraphics[width=3cm]{img/zeitdiskRechteckfunktion}}\\
	& $rect(n)= u(n+N)-u(n-N-1)$& \\
	\textbf{$\mathbb C \text{ oder } \mathbb R$ Sinussignal} & $x(n) = A \sin(\omega n+ \varphi)$& \\
	& $x(n)=Ae^{j(\omega n + \varphi)}$ & \\
