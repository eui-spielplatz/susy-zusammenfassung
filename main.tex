...
\end{tabular}
\raisebox{-.5\height}{\includegraphics[width=6cm]{img/ZeiteigenschaftenVonSystemen}}
...
\begin{tabular}{r >{\centering\arraybackslash} p{6cm}}
	\textbf{linear} & Superpositionsprinzip muss gelten:\\
	& \textbf{homogen}: $T(ax(t))=aT(x(t))$\\
	& \textbf{additiv}: $T(\sum x_i(t))=\sum T(x_i(t))$\\
	& \textbf{durch Ursprung}: $T(0)=0$\\
	\textbf{nichtlinear} & Bedingungen treffen nicht zu\\
\end{tabular}
\paragraph*{Stabilität}
\begin{tabular}{r >{\centering\arraybackslash} p{12cm}}
	\textbf{BIBO-stabil} & $\vert x(t)\vert\leq M_x <\infty\Rightarrow\vert y(t)\vert\leq M_y <\infty$ für alle $t$ und $x(t)$\\
	& beschränktes Eingangssignal $\Rightarrow$ beschränktes Ausgangssignal\\
